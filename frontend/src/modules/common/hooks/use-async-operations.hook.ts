import { AsyncOperationConfigInterface, AsyncOperationStateInterface } from 'modules/common/interfaces';
import { useCallback, useState } from 'react';

export interface UseAsyncOperationsConfigMapInterface {
  [key: string]: AsyncOperationConfigInterface
}

export interface UseAsyncOperationsCurrentOperationInterface<
  TOperationConfigMap extends UseAsyncOperationsConfigMapInterface,
  TKey extends keyof TOperationConfigMap
  > extends AsyncOperationStateInterface<
  Parameters<TOperationConfigMap[TKey]['callback']>[0],
...
export type UseAsyncOperaionsStatesMapType<TOperationConfigMap extends UseAsyncOperationsConfigMapInterface> = {
  [TKey in keyof TOperationConfigMap]?: AsyncOperationStateInterface<
    Parameters<TOperationConfigMap[TKey]['callback']>[0],
    ReturnType<TOperationConfigMap[TKey]['callback']>
  >;
}
...
export const useAsyncOperations = <TOperationConfigMap extends UseAsyncOperationsConfigMapInterface>(
  ...
  const onConfirm = useCallback(
    () => executeOperation(currentOperationName, operationStates[currentOperationName].params),
  ...
  const resetCurrentOperationState = useCallback(
    () => {
      resetOperationState(currentOperationName);
      setCurrentOperationName(null);
    },
