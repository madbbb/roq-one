...
export class ImportService {
  ...
  async importData(data: Record<string, unknown>, flush = false): Promise<void> {
    const savedEntities = {};
    ...
    const sortedData = this.sortEntities(data, this.connection);
    this.logger.log({
      type: LoggingTypeEnum.importData,
    ...
    if (flush) {
      ...
      await this.removeEntities(sortedData, this.connection);
      this.logger.log({
        type: LoggingTypeEnum.importData,
        message: 'Removing of entities started',
      });
    ...
    for (const entityName of entityNames) {
      ...
      const entitiesWithRelations = await this.seedRelations(entityName, importData, this.connection, savedEntities);

      const newEntities = await this.createOrGetEntities(entityName, entitiesWithRelations, this.connection);
      savedEntities[entityName] = await this.connection.manager.save(newEntities, { chunk: 100 });
      this.logger.log({
        type: LoggingTypeEnum.importData,
